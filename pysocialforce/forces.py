...
class DesiredForce(Force):
    ...
    """
    def __init__(self,factor_d):
        super().__init__()
        self.factor_d=factor_d

    def _get_force(self):
        relexation_time = 0.5   #0.5
        goal_threshold = 0.3 #0.3
        pos = self.peds.pos()
        vel = self.peds.vel()
        goal = self.peds.goal()
...
class ObstacleForce(Force):
    ...
    def _get_force(self):
        ...
        threshold = self.config("threshold", 0.2)  #change!
        force = np.zeros((self.peds.size(), 2))
