...
class Simulator:
    ...
    """
    # tau_value,
    def __init__(self, state,tau_value,max_speed_value,multi_speed,factor_d,factor_s, groups=None, obstacles=None, config_file=None):
        ...
        # initiate agents
        self.peds = PedState(state,tau_value,max_speed_value,multi_speed, groups, self.config)

        # construct forces
        self.forces = self.make_forces(self.config,factor_d,factor_s)

    def make_forces(self, force_configs,factor_d,factor_s):
        """Construct forces"""
        force_list = [
            forces.DesiredForce(factor_d),
            forces.SocialForce(factor_s),
