...
def train_model(dataset,save_folder,name, num_epochs=1500,train_type='early_stopping', train_from_scratch = True):
    loss_train=[]
    loss_valid=[]
    ...
    datasets = ["Zara1", "Hotel", "Zara2", "ETH"]
    print('extract training trajectories', flush=True)
    print()
    all_training_data = []
    for data in datasets:
        if data != dataset:
            all_training_data.append(get_data("Datasets/" + data + "/data_for_model/lidar/whole_" + data.lower() + ".pkl"))
            print(f"{data} added in train")
        else:
            x_test,z_test, y_test = get_data("Datasets/" + data + "/data_for_model/lidar/whole_" + data.lower() + ".pkl")
            print(f"{data} added in test")

    x_train = np.r_[all_training_data[0][0], all_training_data[1][0], all_training_data[2][0]]
    z_train = np.r_[all_training_data[0][1], all_training_data[1][1], all_training_data[2][1]]
    y_train = np.r_[all_training_data[0][2], all_training_data[1][2], all_training_data[2][2]]
    ...
    if train_from_scratch != True:
        model.load_state_dict(torch.load("Weights/pretrained/lidar/CNN_sfm_dec24.pth"))
...
if __name__ == '__main__':
    ...
    train_arr, loss_arr = train_model("Hotel","Weights/Hotel/lidar/finetuning/","finetuning_hotel_leave_one_out", num_epochs, train_from_scratch = False)
